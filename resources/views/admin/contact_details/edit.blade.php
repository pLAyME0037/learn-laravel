        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST"
                        action="{{ route('admin.contact-details.update', $contactDetail) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <select id="user_id"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ old('user_id', $contactDetail->user_id) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <x-input-label for="address"
                                :value="__('Address')" />
                            <x-textarea-input id="address"
                                class="block mt-1 w-full"
                                name="address"
                                required>{{ old('address', $contactDetail->address) }}</x-textarea-input>
                            <x-input-error :messages="$errors->get('address')"
                                class="mt-2" />
                        </div>

                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('admin.contact-details.show', $contactDetail) }}"
