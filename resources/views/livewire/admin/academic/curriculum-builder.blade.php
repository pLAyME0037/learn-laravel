<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6 mb-6 flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $program->name }}</h2>
        ...
        <div class="space-y-8">
            @for ($y = 1; $y <= 4; $y++)
                ...
                <div class="relative">
                    <h3 class="text-lg font-bold text-gray-700 dark:text-gray-300 mb-3 border-b pb-2">Year
                        {{ $y }}
                    </h3>
        ...
        @if ($showAddModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75">
                <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-xl">
                    <h3 class="text-lg font-bold mb-4 dark:text-white">Add Course to Curriculum</h3>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm dark:text-gray-300">Course</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm dark:text-gray-300">Year</label>
                                <input type="number"
                                    wire:model="year"
                                    class="w-full border rounded p-2 dark:bg-gray-900 dark:text-white">
                            </div>
                            <div>
                                <label class="block text-sm dark:text-gray-300">Term</label>
                                <input type="number"
                                    wire:model="term"
                                    class="w-full border rounded p-2 dark:bg-gray-900 dark:text-white">
                            </div>
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button wire:click="addCourse"
                            class="bg-indigo-600 text-white px-4 py-2 rounded">Add</button>
                    </div>
                </div>
            </div>
        @endif
