                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-yellow-700">
                                ...
                                <div class="mt-2 space-y-4">
                                    @foreach ($missingCourses as $term => $courses)
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    @if (count($availableClasses) > 0)
                        <span class="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">
                            {{ collect($availableClasses)->flatten()->count() }} Classes Found
                        </span>
                    @endif
                </div>

                @if (collect($availableClasses)->flatten()->count() > 0)
                    <!-- Grouped by Semester -->
                    <div class="space-y-6">
                        @foreach ([1, 2] as $term)
                            @if ($availableClasses[$term] ?? ([])->isNotEmpty())
                                <div>
                                    <h4 class="font-bold text-gray-900 dark:text-white mb-3 border-b pb-2">
                                        Semester {{ $term }}
                                    </h4>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        @foreach ($availableClasses[$term] as $class)
                                            <label
                ...
                @elseif($program_id)
                    ...
                    <div class="flex flex-col items-center justify-center h-full text-center p-8">
                        ...
                        <p class="text-sm text-gray-500 mt-2 max-w-sm">
                            We couldn't find any class sessions for Year {{ $year_level }} of this program in the
                            selected semester.
                        </p>
